...
Lemma nat_of_pos_Z_to_pos x : nat_of_pos x = `|Z2int (BinNums.Zpos x)|%N.
...
Lemma ZgcdE n d : Z.gcd n (BinNums.Zpos d) = Z.of_nat (div.gcdn `|Z2int n| (nat_of_pos d)).
Proof.
apply: Z.gcd_unique.
{ exact: Zle_0_nat. }
...
case E: ((Z.ggcd _ _).2)=>/=; move: E.
rewrite (surjective_pairing (_.2))=>[] [] <- <-.
move: (Z_ggcd_coprime (nb * nd) (BinNums.Zpos (db * dd))).
move: (Z.ggcd_correct_divisors (nb * nd) (BinNums.Zpos (db * dd))).
move: (Z.ggcd_gcd (nb * nd) (BinNums.Zpos (db * dd))).
case (Z.ggcd _ _)=>g ab /= Hg; case ab=> a' b' [] /= Ha' Hb' CPab'.
