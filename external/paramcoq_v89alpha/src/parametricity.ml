      ...
      Declarations.(match cb.const_body with
        | Def _ ->
            ...
            begin match value with
              | Some value -> translate order evd env (of_constr value)
              | None -> assert false
            end
        | OpaqueDef op ->
...
and rewrite_fixpoints order evdr env (depth : int) (fix : fixpoint) source target typ typ_R acc =
  ...
  debug [`Fix] "path" Environ.empty_env !evdr path;
  ...
  debug [`Fix] "res1 = " env !evdr res1;
  debug [`Fix] "res = " env !evdr res;
  debug [`Fix] "nf_evar res = " env !evdr (Evarutil.nf_evar !evdr res);
  res
...
let rec translate_mind_body name order evdr env kn b inst =
  ...
  in
  debug_evar_map [`Inductive] "translate_mind, evd = \n" !evdr;
  let ctx = Evd.to_universe_context !evdr in
  let res = {
