...
let check_inv p =
  ...
  let check_init, t_check_init =
    Osdp.Utils.profile
      (fun () ->
       let init = Sos.(coeff * !!p - sigma1 * pI1 - sigma2 * pI2 - sigma3 * pI3 - sigma4 * pI4) in
       let ret, _, vals, _ =
         Sos.(solve ~options Purefeas [init; coeff; sigma1; sigma2; sigma4; sigma4]) in
  ...
  let coeff = Sos.make "c" in
  let sigma = Sos.make ~n:4 ~d:4 "s" in
  let sigma0 = Sos.make ~n:4 ~d:6 "s0" in
  ...
  let coeff = Sos.make "c" in
  let sigma = Sos.make ~n:4 ~d:4 "s" in
  let sigma1 = Sos.make ~n:4 ~d:6 "s1" in
