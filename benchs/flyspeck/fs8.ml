...
(* bound on x1 : x1 \in [4, 4.0401] *)
let b1 = Sos.((x1 - 4 / 1) * (40401 / 10000 - x1))
(* bound on x2 : x2 \in [4, 4.0401] *)
let b2 = Sos.((x2 - 4 / 1) * (40401 / 10000 - x2))
(* bound on x3 : x3 \in [7.84, 8] *)
let b3 = Sos.((x3 - 784 / 100) * (8 / 1 - x3))
(* bound on x4 : x4 \in [4, 4.0401] *)
let b4 = Sos.((x4 - 4 / 1) * (40401 / 10000 - x4))
(* bound on x5 : x5 \in [4, 40401] *)
let b5 = Sos.((x5 - 4 / 1) * (40401 / 10000 - x5))
(* bound on x6 : x6 \in [7.84, 8] *)
let b6 = Sos.((x6 - 784 / 100) * (8 / 1 - x6))
