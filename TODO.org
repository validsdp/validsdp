...
** TODO Prove RadBigQ2F_correct 				       :Erik:
** TODO State and prove mantissa_bounded subproof for BigQ2FI	       :Erik:
** TODO Setup TypeClasses for the small new functions (add1, etc.)     :Erik:
** LATER Refinement to remove proofs from coqinterval_infnan	       :Erik:
** TODO Make some benchmarks for the Boolean positivity test (itv_Q) :Pierre:
** TODO Design a reflexive tactic for positivity (Ltac)		       :Erik:
** TODO Fix fail "vm_compute does not support existential variables"   :Erik:
** TODO Proceed refinement of posdef_check_itv_Q (w.r.t map_mx)	     :Pierre:
