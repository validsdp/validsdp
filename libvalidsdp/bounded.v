...
rewrite !Rabs_mult (Rabs_right (_ + _)); first lra.
...
rewrite /hb2 Rabs_pos_eq; first exact: big_sum_Rabs_pos.
...
rewrite !Rabs_mult Rabs_inv.
rewrite (Rabs_right r1); first lra; rewrite (Rabs_right r2); first lra.
...
- move=> i; rewrite ffunE.
  ...
  + apply: (Rle_trans _ _ _ (bE _)).
    rewrite ffunE (_ : widen_ord _ _ = i); first exact/val_inj.
    exact: Rle_refl.
  + apply: (Rle_trans _ _ _ (bounded_prop b2)).
    rewrite (_ : ord_max = i); last exact/Rle_refl.
    apply/val_inj => /=.
