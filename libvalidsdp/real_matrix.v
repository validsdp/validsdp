...
rewrite sqrt_mult_alt; try by apply sqr_ge_0.
...
{ rewrite Hny Rmult_0_r.
  ...
  rewrite Rinv_r; try lra; rewrite Rmult_1_l.
  rewrite (Rmult_comm 2) /Rdiv 2!Rmult_assoc Rinv_l; try ring.
  by move=> H; apply Nzxy, (Rmult_eq_reg_l 2); [rewrite Rmult_0_r|lra]. }
apply Rabs_le_inv.
rewrite -(Rabs_pos_eq (_ * _)%Re); try by apply Rmult_le_pos; apply normP_pos.
apply Rsqr_le_abs_0.
rewrite Rsqr_mult.
apply (Rmult_le_reg_r (/ (Rsqr (normP y))));
  [by apply Rinv_0_lt_compat, Rlt_0_sqr|].
rewrite Rmult_assoc Rinv_r; try by apply Rgt_not_eq, Rlt_0_sqr.
rewrite Rmult_1_r.
...
repeat (rewrite sqrt_def; try by apply dotprodP_pos).
...
{ apply Mle_norm2 => i j; rewrite !mxE.
  rewrite Rabs_pos_eq; try by apply big_sum_Rabs_pos.
  rewrite Rabs_pos_eq; try by apply Rmult_le_pos; apply sqrt_pos.
...
rewrite /norm2 /normFrobenius -sqrt_mult;
  try (by apply pos_INR);
  try (by apply big_sum_pos_pos => i; apply big_sum_pos_pos => j; apply Rle_0_sqr).
apply f_equal; rewrite big_distrr /= /dotprod mxE.
apply /eq_bigr => i _; rewrite !mxE.
rewrite -sqrt_mult;
  try (by apply pos_INR); try (by apply big_sum_pos_pos => i'; apply Rle_0_sqr).
...
apply HA.
rewrite norm2_scale_pos; try by apply Rlt_le.
by rewrite Rinv_l; try apply Rgt_not_eq, norm2_def_contrap.
Qed.
