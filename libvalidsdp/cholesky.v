...
replace (\sum__ _) with (\sum_i [ffun i => (a i * a i)%Re] i);
  [|by apply eq_bigr => i _; rewrite ffunE].
rewrite -{2}big_Rabs_pos_eq; try (by move=> i; rewrite ffunE; apply sqr_ge_0).
rewrite -{2}(Rabs_pos_eq (_ ^ 2)); try (by apply pow2_ge_0).
...
{ apply Rmult_le_compat; [by apply Rabs_pos|by apply Rabs_pos| |].
  { apply (Rle_trans _ _ _ (Rabs_triang _ _)).
    ...
    rewrite Rabs_pos_eq; try (lra); apply Rplus_le_compat_l, bounded_prop. }
  by apply bounded_prop. }
ring_simplify; rewrite /pow Rmult_1_r sqrt_def;
  first [by right; simpl; ring
        |by apply Rplus_le_le_0_compat; move: (eps_pos fs); lra].
Qed.
...
{ rewrite (@big_addn _ _ _ 0 n.+1 i.+1) big_mkord.
  ...
  rewrite !mxE ifF; try (by rewrite GRing.mul0r).
  rewrite inordK; try (by rewrite /leq -addSnnS addnK).
...
{ apply eq_bigr => k _; rewrite !mxE !ffunE.
  ...
  { by rewrite ifT; try (by []); rewrite ifT; try (by apply (leq_trans H)). }
  rewrite inordK;
    first [by apply ltnW; apply ltn_ord|by apply ltn_trans with i; apply ltn_ord]. }
have H : (@inord n i = i)%N by rewrite inord_val.
by rewrite !mxE ifT; try (rewrite ifT); rewrite H.
Qed.
...
{ rewrite (@big_addn _ _ _ 0 n.+1 i.+1) big_mkord.
  apply big_rec => [//|k x _ ->]; rewrite GRing.addr0.
  rewrite !mxE ifF; try (by rewrite Rabs_R0 GRing.mul0r).
  rewrite inordK; try (by rewrite /leq -addSnnS addnK).
  move: k; case i => iv ip /=; case=> kv kp /=.
  by rewrite -(subnK ip) ltn_add2r. }
rewrite H {H} GRing.addr0 big_mkord big_ord_recr /= Rplus_comm; apply f_equal2.
{ apply eq_bigr => k _; rewrite !mxE !ffunE.
  suff H : (@inord n k <= i)%N.
  { by rewrite Rabs_mult ifT; try (by []);
      rewrite ifT; try (by apply (leq_trans H)). }
  rewrite inordK;
    first [by apply ltnW; apply ltn_ord|by apply ltn_trans with i; apply ltn_ord]. }
have H : (@inord n i = i)%N by rewrite inord_val.
by rewrite Rabs_mult !mxE ifT; try (rewrite ifT); rewrite H.
Qed.
...
rewrite -(@alpha_iltj j j) //= Rmult_1_r /d sqrt_def; try (by right).
apply Rmult_le_pos; [|apply Rplus_le_le_0_compat].
{ have H : (alpha j j < 1)%Re;
    [by rewrite alpha_iltj; first [by apply Hj|by []]|].
...
rewrite 3!mxE mulmx_dotprod trmxK dotprod_rt_i_j; try (by apply ltnW).
set (c := A i j).
...
rewrite [X in Rabs X](_ : _ =
  ...
  try (by rewrite /GRing.add /GRing.opp /c /= -[FS_of (format fs)]/F; ring).
rewrite Rabs_minus_sym; apply (Rlt_le_trans _ _ _ (lemma_2_1 a b c Hbk)).
rewrite /bk -Hrtij; apply Rplus_le_compat.
{ rewrite alpha_iltj; try (by apply ltnW).
  rewrite dotprod_Mabs_rt_i_j; try (by apply ltnW).
...
rewrite Rabs_pos_eq;
  first [by apply Hmaxdiag'|by apply Rlt_le, (proj2 HAR)].
Qed.
...
rewrite 3!mxE mulmx_dotprod trmxK dotprod_rt_i_j; try (by apply leqnn).
set (c := A i i).
set (a := [ffun k : 'I_i => Rt (inord k) i]).
...
{ rewrite alpha_iltj //; apply Rmult_le_compat_l; [by apply INR_eps_pos|].
  ...
  rewrite Rplus_comm Rabs_pos_eq; try (by apply sqr_ge_0); f_equal.
  by apply eq_bigr => k; rewrite !ffunE Rabs_pos_eq; try (apply sqr_ge_0). }
rewrite (Rmult_assoc _ eta) (Rmult_comm eta) -Rmult_assoc.
apply Rmult_le_compat_r; [by apply eta_pos|].
apply Rmult_le_compat; try apply Rplus_le_le_0_compat;
...
rewrite -{3}norm2_const -norm2_scale_pos; try (lra); apply f_equal.
...
{ rewrite [X in X *m _]mx11_scalar [X in _ *m X]mx11_scalar -scalar_mxM.
  ...
  apply Rsqr_le_abs_1; rewrite (Rabs_pos_eq (norm2 _)); try (by apply norm2_pos).
...
{ rewrite /GRing.mul /GRing.add /Rdiv /= Rmult_1_r (Rmult_assoc _ (/ _)).
  ...
  rewrite /dv !mxE /In1 /d sqrt_def; try (by apply Rmult_le_pos).
...
by rewrite /In1 norm2_const sqrt_def; try (by apply pos_INR).
Qed.
