...
{ apply (Rle_trans _ _ _ (Rplus_le_compat_r _ _ _ B1)).
  have->: \sum_(i <- merge leq sl sr) Rabs (x (inord i)) = s1t + s2t.
    by rewrite -big_cat; apply: perm_big; rewrite perm_merge.
  fold_eps1; rewrite -!subn1 !minus_INR ?plus_INR; try exact/ltP.
  apply: Req_le; ring. }
...
{ have Hs21t': s2t <= s1t.
  ...
  { have Heps := epsd1peps_pos  (eps_pos fs).
    ...
    rewrite -subn1 minus_INR ?[INR 1]/=; try lra.
    exact/leP. }
  { ring_simplify.
...
rewrite (_: ?[d1] + s1t + (?[d2] + s2t) = ?d1 + s2t + (?d2 + s1t)); try ring.
apply: Rplus_le_compat.
{ apply (Rle_trans _ _ _ (Rplus_le_compat_r _ _ _ Hdelta1)).
...
rewrite (_: ?[x] - ?[o] = srx - ?o + (?x - srx)); try ring.
apply: (Rle_trans _ _ _ (Rabs_triang _ _)).
...
{ apply (Rle_trans _ (\sum_(i <- sn) (eps / (1 + eps) * Rabs (x (inord i)) + eta)%Re)).
  { apply: Rle_big_compat => i.
    ...
    rewrite (_: _ - _ = d * x (inord i) + e)%Re; try ring.
