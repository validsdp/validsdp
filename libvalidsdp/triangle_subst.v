...
have fgei (i' : 'I_n.+1) k :
  ...
  rewrite -IHk; first by rewrite ltnW.
...
rewrite /fi (take_nth ord0) ?size_ord_enum// foldl_rcons mxE ifF.
...
rewrite [X in fi _ X]ie fgei; first by rewrite inordK ?subnKC// ltnW.
...
- rewrite /eFn' ffunE /eFn ffunE; case: splitP => k jk; last first.
  ...
  have [yhat'0 | /eqP yhat'n0] := (yhat' (inord j) ord0 : R) =P 0.
    ...
    rewrite inordK; first by apply: leq_trans (ltn_ord k); rewrite -jk/=.
