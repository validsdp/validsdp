...
\paragraph*{Rump/Jeannerod Lemma 2.1} (LibValidSDP lemma\_2\_1): Analysis of $(c-\sum a_i b_i)/b_k$.
  \newline
...
& \qquad
\cdot \sqrt{(A_{jj}+2j\lambda\epsilon)/(1-(j+2)\epsilon)}
+ 4 \lambda\epsilon (n+1+a_\mathrm{max}) \\
& \le
(n+1)\epsilon
...
& \le n\epsilon \sqrt{(A_{ii}+2i\lambda\epsilon)/(1-(i+2)\epsilon)} \sqrt{(A_{jj}+2j\lambda\epsilon)/(1-(j+2)\epsilon)} \quad\qquad\mathrm{(by 2.3aux1)}\\
