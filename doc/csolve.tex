...
\paragraph*{Theorem Demmel\_L4\_adapted:} (lemma\_4 in triangle\_subst.v in LibValidSDP)
Suppose $L$ is an $n \times n$ lower triangular (i.e., for all $i < j$,
$L_{ij} = 0$) floating-point matrix with non zero diagonal elements
(i.e., for all $i$, $L_{ii} \neq 0$) and $b$ is a floating-point vector.
...
Let $a_\mathrm{max}$ be greater or equal to all diagonal elements of $A$
and assume $a_\mathrm{max} \geq 0$.
Then
...
\begin{equation}\tag{cholesky\_back\_err}
\begin{array}{rl}
|\delta A_{ij}| & <
...
\paragraph*{Lemma.} We obtain bounds on $\delta b'$ by
combining (demmel\_lemma\_4) and (2.3aux1):
\begin{equation}\tag{csolve\_back\_err\_aux3}
\begin{array}{rl}
|\delta b'_i| & \le (1+n\epsilon)\lambda \epsilon (n + |\hat{R}^T_{ii}|) \\
& \le (1+n\epsilon)\lambda \epsilon (n + \norm{\hat{R}_i}_2) \\
& \le (1+n\epsilon)\lambda \epsilon (n + \sqrt{a'_\mathrm{max}}) \\
& \le (1+n\epsilon)^2\lambda \epsilon (n + \sqrt{a'_\mathrm{max}}).
\end{array}
\end{equation}

\paragraph*{Lemma.} We obtain bounds on $(\hat{R}^T + \delta R^T) \delta y$ by
combining (demmel\_lemma\_4) and (2.3aux1):
\begin{equation}\tag{csolve\_back\_err\_aux4}
\begin{array}{rl}
|((\hat{R}^T + \delta R^T) \delta y)_i| & \le (|\hat{R}^T + \delta R^T| \cdot |\delta y|)_i \\
& \le \sum_k |\hat{R}^T + \delta R^T|_{ik} |\delta y|_k \\
& \le \sum_k (1+n\epsilon)|\hat{R}_{ki}| \cdot |\delta y|_k \qquad \mathrm{(by~demmel\_lemma\_4)} \\
& \le (1+n\epsilon) \norm{\hat{R}_i}_2 \norm{\delta y}_2 \qquad \mathrm{(by~Cauchy-Schwartz)} \\
& \le (1+n\epsilon) \sqrt{a'_\mathrm{max}} \norm{\delta y}_2 \qquad \mathrm{(by~2.3aux1)} \\
& \le (1+n\epsilon) \sqrt{a'_\mathrm{max}} n \norm{\delta y}_\infty \\
& \le (1+n\epsilon) \sqrt{a'_\mathrm{max}} n (1+n\epsilon) \lambda \epsilon (n + \sqrt{a'_\mathrm{max}}) \\
& = (1+n\epsilon)\lambda \epsilon (n + \sqrt{a'_\mathrm{max}}) (1+n\epsilon)n\sqrt{a'_\mathrm{max}} \\
& \le (1+n\epsilon)^2\lambda \epsilon (n + \sqrt{a'_\mathrm{max}}) n\sqrt{a'_\mathrm{max}}.
\end{array}
\end{equation}
...
Taking into account underflows in both the factorization and the substitution, we have
that there is a mixed error $\delta A$ and $\delta b$ such that
$(A+\delta A)\hat{x}=b+\delta b$, bounded as follows:
...
|\delta b| & \le & (1+n\epsilon)^2 (n+1)^2 \lambda \epsilon (1 + a'_\mathrm{max})
\end{array}
\end{equation}

\paragraph*{Proof.}
Since $\hat{y} + \delta y = (\hat{R} + \delta R)\hat{x}$, we have
...
\begin{equation}\tag{xxx}
  \begin{array}{rl}
    \delta b   & = \delta b' +(\hat{R}^T + \delta R^T) \delta y.
  \end{array}
\end{equation}

From (cholesky\_back\_err) we have
...
From csolve\_back\_err\_aux2 we have
 ...
 \le n^2\epsilon^2 a'_\mathrm{max}$.
\newline
The result follows using csolve\_back\_err3 and csolve\_back\_err4 and $\sqrt{a'_\mathrm{max}} \le 1 + a'_\mathrm{max}$.  Q.E.D.
