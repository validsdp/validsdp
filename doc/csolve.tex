...
\begin{abstract}
  Following Demmel (1981) with some lemmas improved by Rump and Jeannerod (2014)
  and previously implemented in libValidSDP by Roux (2017), we derive
  the backward error analysis for solving a s.d.p. linear system $Ax=b$
...
As we explain in the next sections, we make some adjustments to the
...
|\delta A| \le ((3n+1)\epsilon+n^2\epsilon^2)\frac{a_\mathrm{max}+2(n-1)\lambda\epsilon)}{1-(n+1)\epsilon}+4 \lambda\epsilon (n+1+a_\mathrm{max}).
\end{equation}
...
$|(\delta R^T\hat{R})_{ij}| \le n\epsilon (a_\mathrm{max}+2(n-1)\lambda\epsilon)/(1-(n+1)\epsilon)$\newline
and similarly 
$|(\hat{R}^T\delta R)_{ij}| \le n\epsilon (a_\mathrm{max}+2(n-1)\lambda\epsilon)/(1-(n+1)\epsilon)$.\newline
