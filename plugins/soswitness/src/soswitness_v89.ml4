...
let ofInt31 c =
  ...
  aux (snd (Constr.decompose_app c)) 0
...
let soswitness_opts ?(intro=false) c opts =
  let opts = List.map (* ofList *) ofParameters opts in
  Format.printf "len: %d@." (List.length opts);
  let options =
    List.fold_left
      ...
      Sos.default opts in
  try
    let c = soswitness intro options c in
    let () = Format.printf "  *** success@." in
    let res = Proofview.tclUNIT (Value.of_constr c) in
    let () = Format.printf "  *** apres of_constr@." in
    res
  with e ->
    let () = Format.printf "  *** exception@." in
    Proofview.tclZERO e (* SosFail (level, msg) -> failtac level msg
...
let () = define2 "soswitness2" constr (list constr) begin fun c opts ->
  let c = EConstr.Unsafe.to_constr c in
  let opts = List.map EConstr.Unsafe.to_constr opts in
  soswitness_opts c opts (* >>= fun c -> Proofview.tclUNIT (Value.of_constr c) *)
end
