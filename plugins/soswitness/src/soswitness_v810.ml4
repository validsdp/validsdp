...
(* FIXME: still needed while Int63, Bignums and Interval don't
   implement the appropriate Register commands. *)
let init_constant' dir s =
  let find_constant contrib dir s =
    Globnames.printable_constr_of_global
      (Coqlib.find_reference contrib dir s) in
  let contrib_name = "soswitness" in
...
let int63_path = ["Coq"; "Numbers"; "Cyclic"; "Int63"; "Int63"]
let coq_int63 = lazy (init_constant' int63_path "int")
...
let mkInt63 n =
  let h = Z.(to_float (n asr 32)) in
  let l = Z.(to_float (n land z_mask32)) in
  let ui = Uint63.(l_or (l_sl (of_float h) uint63_32) (of_float l)) in
  Constr.(of_kind (Int ui))
  
let ofInt63 c = match Constr.kind c with
  | Constr.Int i ->
     let h = Uint63.(to_float (l_sr i uint63_32)) in
     let l = Uint63.(to_float (l_and i uint63_mask32)) in
     Z.(((of_float h) lsl 32) lor (of_float l))
  | _ -> raise Parse_error
    ...
    with Parse_error ->
      ...
      fail maxlevel
        Pp.(str "soswitness: wrong input type (expected "
            ++ Printer.pr_constr_env Environ.empty_env Evd.empty ty_input
            ++ str ").") in
